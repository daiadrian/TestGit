...
​		安装好的 nginx 目录在 /etc/nginx 下，默认的配置文件是 /etc/nginx/nginx.conf 

​		如果是要配置默认的 80 端口，要注释掉默认配置文件中的：`include /etc/nginx/conf.d/*.conf;`   因为这里包含了默认的 root 、index 等配置，会影响自己配置的80端口相关的内容
