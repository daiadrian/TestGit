...
- CA：单点集群，满足一致性，可用性系统；通常在可扩展性上不强（RDBMS关系型数据库）
- CP：满足一致性，分区容错性系统；通常性能不是很高（MongoDB、HBase、Redis）
- AP：满足可用性和分区容错性系统；通常对一致性要求低一些（CouchDB）
...
目标：每个服务调用3次后再进行轮询（调用次数不是很对，懒得改了)
